...
Future<EditImageInfo> cropImageDataWithDartLibrary(
  ...
  logger.d('dart library start cropping');
  ...
  if (src != null) {
    ...
    src.frames = src.frames.map((Image image) {
      ...
      logger.d('${time3.difference(time2)} : crop/flip/rotate');
      return image;
    }).toList();
  ...
  logger.d('${time5.difference(time4)} : encode');
...
Future<EditImageInfo> cropImageData(
  ...
  return cropImageDataWithDartLibrary(controller);
}
